---
name: clean-code
description: 'Write and review readable, maintainable code through disciplined naming, cohesive functions, clear responsibilities, and clean error handling. Use during every substantive code implementation, modification, refactoring, or review, even when the user does not explicitly request code-quality analysis; skip generated code and purely mechanical, configuration-only, formatting-only, or data-only changes. Covers SRP, comment discipline, formatting, error handling, and unit-test quality. For refactoring techniques, see refactoring-patterns. For architecture and dependency rules, see clean-architecture.'
license: MIT
metadata:
  author: wondelai
  version: "1.4.0"
